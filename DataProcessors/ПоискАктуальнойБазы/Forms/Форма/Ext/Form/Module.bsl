// 30.07.19
// +команда ОтметитьБазуКакПригоднуюДляОбновленияНаСервере
// +Вы учерены что хотите получить инфо о базах через ком-соединение???
	...
	Для НомерСтроки = 1 По ТекстовыйДокИзФайла.КоличествоСтрок() Цикл
		...
		Если ЗначениеЗАполнено(СтруктураБазы.ИмяБазы)
			...
			Если НайденныеБазы.НайтиСтроки(Новый Структура("ИмяБазы", СтруктураБазы.ИмяБазы)).Количество() = 0 Тогда
				...
				// Проверим создан ли в корне базы файл CORRECT
				ФайлТриггерАктуальности = Новый Файл(СтруктураБазы.ПутьКБазе+"\CORRECT");
				Если НЕ ФайлТриггерАктуальности.Существует() Тогда
					СтрокаБазы.АктуальнаяБазаДляОбновления = Ложь;
				Иначе
					СтрокаБазы.АктуальнаяБазаДляОбновления = Истина;
				КонецЕсли; 
...
Процедура ПолучитьПараметрыКонфигураций(Команда)
	...
	//======================================================================================================================
	ТекстВопроса = "Подключение к каждой базе и перебор авторизационных данных может занять долгое время. Продолжить?";
	ПоказатьВопрос(Новый ОписаниеОповещения("ПолучитьДанныеИзБазы", ЭтаФорма, ), // TODO: для 8.3 использовать ЭтотОбъект!!!
						ТекстВопроса,
...
&НаКлиенте
Процедура ПолучитьДанныеИзБазы(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ОтметитьБазуКакПригоднуюДляОбновления(Команда)
	ОтметитьБазуКакПригоднуюДляОбновленияНаСервере();
КонецПроцедуры

&НаСервере
// Если в пути к базе имеется файл 1Cv8.1CD - то создадим внутри папки файл
Процедура ОтметитьБазуКакПригоднуюДляОбновленияНаСервере()
	
	СтрокаТаблицыБазы = НайденныеБазы.НайтиПоИдентификатору(Элементы.НайденныеБазы.ТекущаяСтрока);
	
	// Если не поставили курсор в таблице
	Если СтрокаТаблицыБазы = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	
	ПутьКФайловойБазе = СтрокаТаблицыБазы.ПутьКБазе;
	
	Если СтрокаТаблицыБазы.СуществуетФайлБД Тогда
		СоздатьФайлФлаг(ПутьКФайловойБазе, "CORRECT");
	Иначе
		Сообщить("Файла БД не существует по пути в выделенной строке");
	КонецЕсли; 
	
КонецПроцедуры

&НаСервере
Функция СоздатьФайлФлаг(Путь, ИмяТриггера)
	
	ТекстовыйФайл = новый ТекстовыйДокумент;
	ТекстовыйФайл.Записать(Путь+"\"+ИмяТриггера);
